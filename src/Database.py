...
import datetime
import collections
from src.WebServiceData import WebServiceData
import traceback
class DatabaseAccesser:
    ...
    """
    @__Transact
    def Insert(self, data:WebServiceData):
        url = self.__GetServiceUrl(url)
    ...
    新規挿入時はWebServiceDataを返す。更新時はNoneを返す。
    と思ったが、挿入時は<img>追加したいし、更新時はhtml更新したい。どうすれば…。
    """""
    @__Transact
    ...
    DBに挿入する。
    """
    def __InsertService(self, data:WebServiceData):
    ...
    DBに挿入する。
    """
    def __InsertFavicon(self, data:WebServiceData, service_id=None):
    ...
    DBに挿入する。
    """
    def __InsertOldFavicon(self, data:WebServiceData, service_id=None):
        if None is service_id:
            service_id = self.__services['Services'].find_one(Url=data.Url)['Id']
    ...
    レコードを更新する。
    """
    def __UpdateService(self, data:WebServiceData):
    ...
    レコードを更新する。
    """
    def __UpdateFavicon(self, data:WebServiceData, service_id):
        if None is service_id:
            service_id = self.__services['Services'].find_one(Url=data.Url)['Id']
    ...
    def __GetServiceUrl(self, url):
        u = urllib.parse.urlparse(url)
        if '/' == u.path:
            return url[:-1] # python3.6.1で https://qiita.com/ だと https:qiita.com のように://が壊れる。ドメイン＋末尾スラッシュのみで発生。3.4.3では問題なかった。バグ？仕様？
        else:
            return urllib.parse.urlunparse(u).replace(u.path, '')
