...
class WebServiceData:
    ...
    def __GetSoupForUrllib(self):
        return BeautifulSoup(urllib.request.urlopen(self.Url), 'html.parser')
...
#        return BeautifulSoup(r.text, 'lxml')
    ...
    CSSは動的に変更できないので。
    """
    def GetImgString(self):
#        return '<a href="{href}" title="{title}"><img src="{data_uri_base64}" width="16px" height="16px"></img></a>'.format(
        return '<a href="{href}" title="{title}"><img class="base64-icon {class_name}" src="{data_uri_base64}"></img></a>'.format(
            href=self.Url,
            title=self.Title,
            class_name=self.Classname,
            data_uri_base64=self.DataUri)
    
    def GetCssDict(self):
        return {"width": "16px", "height": "16px", "background-size": "16px 16px", "background-image": "url({0})".format(self.Base64)}
        
    def GetCssString(self):
        return """
.base64-icon.{class_name} {{
    width: 16px;
    height: 16px;
    background-size: 16px 16px;
    background-image: url({base64});
}}
""".format(class_name=self.Classname, base64=self.Base64)
    ...
    def __GetUrl(self, url):
        ...
        if '/' == u.path:
            self.__url = url[:-1] # python3.6.1で https://qiita.com/ だと https:qiita.com のように://が壊れる。ドメイン＋末尾スラッシュのみで発生。3.4.3では問題なかった。バグ？仕様？
        else:
